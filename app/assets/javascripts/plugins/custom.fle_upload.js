  ...
  jQuery.fn.customFile = function() {
    return this.each(function() {
      var $file = jQuery(this).addClass("custom-file-upload-hidden"), // the original file input
        $wrap = jQuery(
          '<div class="input-group"><span class="input-group-addon"><i class="fa fa-paperclip"></i></span>'
        ),
        $input = jQuery(
          '<input type="text" class="form-control file-upload-input text-left" />'
        ),
        // Button that will be used in non-IE browsers
        $button = jQuery(
          '<span class="input-group-btn"><button type="button" class="file-upload-button btn btn-primary">' +
            "Attachment" +
            "</button></span>"
        ),
        // Hack for IE
        $label = jQuery(
          '<label class="file-upload-button" for="' +
            $file[0].id +
            '">' +
            jQuery(this).attr("data-btn-text") +
            "<</label>"
        );
