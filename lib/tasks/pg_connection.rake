...
namespace :export do
    ...
    task datawarehouse: :environment do
        ...
        conn = PG::Connection.open(host: "codeboxx-postgresql.cq6zrczewpu2.us-east-1.rds.amazonaws.com", port: 5432, dbname: "raphael_vallee", user: "codeboxx", password: "Codeboxx1!")
        ...
        Intervention.all.each do |intervention|
            conn.exec("INSERT INTO \"factintervention\"(id, user_id, building_id, battery_id, column_id, elevator_id, intervention_start, intervention_finish, intervention_result, report, intervention_status) VALUES 
            (#{intervention.id}, #{intervention.user_id}, #{intervention.building_id}, #{intervention.battery_id.blank? ? 'NULL' : intervention.battery_id}, #{intervention.column_id.blank? ? 'NULL' : intervention.column_id},
            #{intervention.elevator_id.blank? ? 'NULL' : intervention.elevator_id}, '#{intervention.intervention_start}', '#{intervention.intervention_finish}', '#{intervention.intervention_result}', '#{intervention.report}', '#{intervention.intervention_status}')")
        end
